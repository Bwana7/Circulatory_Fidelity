...
\begin{center}
{\large Version 1.1 --- January 13, 2026}
\end{center}
\vspace{-1em}
...
In this decomposition, $D$ (diagonal) captures \emph{nodal} structure and $R$ (off-diagonal) captures \emph{relational} structure. MFVI keeps $D$ but discards $R$. Building on the classical result that Gaussian mutual information $I(Z;X) = -\frac{1}{2}\log(1-\rho^2)$ depends \emph{only} on correlation $\rho$---not on marginal variances \citep{gelfand1959calculation,cover2006elements}---we develop \textbf{Inference Coupling} $\IC = |\rho|$ as a pre-inference diagnostic. The companion \textbf{Balance Factor} $\BB$ characterizes architectural symmetry but adds no predictive value for MFVI failure ($\Delta R^2 \approx 0$), empirically confirming the theoretical invariance. \textbf{Control Coupling} $\CC = |\rho| \cdot (\sigma_i/\sigma_j)$ captures directional intervention leverage, where scale \emph{does} matter---this is where the ``bottleneck'' principle applies.

We establish three novel theoretical contributions and one formalization: (1) The \textbf{Computational Synergy Principle}---synergistic dependencies arise when the generative function is affine over $\text{GF}(2)$, connecting Siegenthaler's correlation immunity from cryptography to information-theoretic synergy, and enabling a two-stage diagnostic where pairwise $\IC \approx 0$ combined with interaction $\IC > 0$ identifies XOR-type structure. (2) The \textbf{Dependency Asymmetry}---high IC predicts MFVI failure in filtering models but hierarchy redundancy in pooling models, resolved by a novel taxonomy distinguishing constitutive from inductive coupling. (3) The \textbf{Proximal Dominance Principle}---formalizing documented observations that proximal layers dominate in hierarchical models, we prove this reduces MFVI diagnostic complexity from $O(L^2)$ to $O(1)$; proximal coupling causes up to $40\times$ degradation while distal coupling causes exactly $1.0\times$ (mathematically guaranteed). (4) The \textbf{Maximal Coupling Rule}---for non-stationary series, MFVI suitability depends on $\IC_{\max}$, not global average.

Validation spans stochastic volatility filters, hierarchical linear models, and three-layer hierarchies, yielding a practical pre-inference workflow. The framework clarifies a fundamental asymmetry: \emph{what you can learn} from a relationship (inference) depends only on coupling strength, while \emph{what you can do} through a relationship (control) depends on both coupling and scale.
\end{abstract}
...
\textbf{Historical note:} The original v1.0 formulation defined $\text{CF} = I(Z;X)/\min(H(Z), H(X))$. The standard result that marginal variances cancel in Gaussian mutual information (Proposition~\ref{prop:gaussian-mi}) proves this normalization is unnecessary for inference diagnostics. The current formulation uses IC = $|\rho|$ as the primary diagnostic, equivalent to the Linfoot correlation.
\end{tcolorbox}
...
A classical result (Proposition~\ref{prop:gaussian-mi}) shows that for Gaussian systems, the mutual information---and hence the KL cost of mean-field factorization---is:
\begin{equation}
...
\paragraph{Contributions.} This work synthesizes established results from information theory into a practical diagnostic framework, contributing three novel theoretical results and one novel formalization:
\begin{enumerate}
    \item \textbf{The Computational Synergy Principle and Two-Stage Protocol:} We prove that synergy-generating functions are characterized algebraically as affine over $\text{GF}(2)$, establishing a novel connection between Siegenthaler's correlation immunity \citep{siegenthaler1984correlation} from cryptography and information-theoretic synergy \citep{williams2010nonnegative}. This equivalence enables a two-stage diagnostic: pairwise IC detects standard coupling, while $\IC_{\text{pairwise}} \approx 0$ combined with $\IC_{\text{interaction}} > 0$ positively identifies XOR-type structure (Theorem~\ref{thm:synergy}, Section~\ref{sec:synergy})
    \item \textbf{The Dependency Asymmetry and Structural Taxonomy:} We identify that high IC predicts MFVI failure in filtering models but hierarchy redundancy in pooling models---an asymmetry not previously noted despite both phenomena being individually documented \citep{wang2004convergence,turner2011two}. We resolve this through a novel taxonomy distinguishing \emph{constitutive coupling} (load-bearing, as in state-space models) from \emph{inductive coupling} (scaffolding, as in hierarchical models) (Section~\ref{sec:taxonomy})
    \item \textbf{The Proximal Dominance Principle:} We formalize the empirically documented observation \citep{havtorn2021hierarchical,sonderby2016ladder,zhao2017infovae} that proximal layers dominate in hierarchical models, proving that for MFVI diagnostics this reduces complexity from $O(L^2)$ to $O(1)$. Proximal coupling drives up to $40\times$ degradation (95\% CI: 2.7--102) while distal coupling causes exactly $1.0\times$ (mathematically guaranteed). For non-stationary time series, we derive the corollary \emph{Maximal Coupling Rule}: MFVI suitability is determined by $\IC_{\max}$, not global average (Sections~\ref{sec:deep},~\ref{sec:windowed})
    \item \textbf{The Relational Diagnostic Framework:} Building on the standard result that Gaussian mutual information depends only on correlation \citep{gelfand1959calculation,cover2006elements}, we develop a diagnostic interpretation: MFVI failure depends on \emph{relational} structure (IC), not \emph{nodal} structure (marginal variances). The Balance Factor $\BB$ characterizes architecture but adds $\Delta R^2 \approx 0$ for predicting MFVI failure---an empirical confirmation of the theoretical invariance. Control Coupling $\CC$ captures directional intervention leverage where scale \emph{does} matter. (Section~\ref{sec:definition})
\end{enumerate}
The framework connects IC to the Linfoot informational correlation \citep{linfoot1957informational}, inheriting its desirable properties: coordinate invariance, boundedness on $[0,1]$, and extension to non-Gaussian distributions (Section~\ref{sec:linfoot}). These contributions are supported by empirical validation ($N = 32{,}000$+ simulations) and synthesized into a practical prior predictive workflow (Section~\ref{sec:workflow}).
...
\begin{itemize}
    \item \textbf{Section~\ref{sec:definition}: The Relational Diagnostic Framework.} We establish the precision matrix foundation, define IC and companion metrics ($\BB$, $\CC$), present the standard mutual information formula with its diagnostic interpretation, and connect IC to the Linfoot correlation.
    \item \textbf{Section~\ref{sec:geometry}: Information-Geometric Interpretation.} We connect IC to KL projection cost on statistical manifolds, showing that IC determines the normalized KL gap for Gaussian posteriors.
...
\end{itemize}
\section{The Relational Diagnostic Framework}
\label{sec:definition}
%=============================================================================

This section establishes the theoretical foundation of Circulatory Fidelity: MFVI failure depends entirely on \emph{relational} structure (coupling between variables), not on \emph{nodal} structure (individual variable properties). This insight---grounded in classical information theory---leads to Inference Coupling (IC) as the primary diagnostic, with companion metrics for architectural characterization and control analysis.
...
\subsection{The Relational Invariance Property}
\label{sec:relational-invariance}

The central result underlying IC comes from classical information theory: Gaussian mutual information depends only on correlation, not on marginal scales \citep{gelfand1959calculation,cover2006elements}.
...
\end{align}
The marginal variances cancel completely. This calculation appears in standard texts \citep{cover2006elements}; we include it here to make the cancellation explicit.
\end{proof}

\paragraph{Diagnostic interpretation.} Though the formula itself is classical, its \emph{diagnostic implication} for variational inference has not been previously emphasized: any metric normalizing by marginal entropies adds no predictive value beyond $|\rho|$. The min-entropy normalization was conceptually motivated by a ``bottleneck'' intuition, but since the $\sigma$ terms cancel in mutual information, the bottleneck affects only what you can \emph{do} (control), not what you can \emph{learn} (inference). We term this the \textbf{relational invariance property}: inference depends on relational structure alone.
...
\textbf{Critical empirical finding:} When added to IC in regression models predicting MSE ratio, $\BB$ contributes $\Delta R^2 \approx 0.00001$. The Balance Factor characterizes architecture but has \emph{no predictive value} for MFVI failure. This confirms the relational invariance property empirically.
...
\subsection{Connection to Linfoot Informational Correlation}
\label{sec:linfoot}

IC inherits strong theoretical justification from the \textbf{Linfoot informational correlation} \citep{linfoot1957informational}, a dependence measure designed to satisfy Rényi's axioms:
\begin{equation}
r_L(z,x) = \sqrt{1 - \exp(-2 \cdot I(z;x))}
\label{eq:linfoot}
\end{equation}

A well-known property of the Linfoot correlation---established in the original 1957 paper---is that it equals $|\rho|$ for bivariate Gaussians:

\begin{proposition}[Gaussian Linfoot Correlation {\citep{linfoot1957informational}}]
...
\paragraph{Significance for IC.} This established equivalence validates IC as a principled diagnostic: IC (derived from precision matrix structure) coincides with the Linfoot correlation (derived from information-theoretic axioms) for Gaussian systems. The connection provides IC with several desirable properties inherited from the Linfoot framework:
\begin{enumerate}
...
This is a standard result. By the relational invariance property (Section~\ref{sec:relational-invariance}), this projection cost depends \emph{only} on $\rho = \IC$---the marginal variances cancel completely. It follows that IC directly determines the information-geometric cost of mean-field factorization.
...
\end{equation}
This is the KL divergence from the joint distribution to the product of marginals---precisely the KL gap $\Delta_{\text{KL}}$. By the standard Gaussian mutual information formula (Proposition~\ref{prop:gaussian-mi}), this equals $-\frac{1}{2}\log(1-\rho^2)$ where $\rho = \IC$.
\end{proof}
...
\end{align}
Note that the marginal variances $\sigma_Z^2$ and $\sigma_X^2$ cancel completely, confirming the relational invariance property.
\end{proof}
...
This comparison reveals a counter-intuitive asymmetry: non-affine nonlinearities induce detectable marginal dependencies even in complex systems, whereas purely affine functions maintain marginal independence despite complete joint determination. We may characterize this as a \emph{fluid--crystalline} distinction in information topology: Rule~30's complexity is ``fluid''---the OR gate creates state-dependent edges where one input can render another irrelevant, allowing information to leak through porous logical structure. Rule~150's complexity is ``crystalline''---every input is always load-bearing, creating a rigid lattice where all information is holistically encoded. Unstructured complexity is thus more amenable to standard diagnostics than structured synergy; the crystalline order of XOR-type computation evades detection precisely because of its algebraic perfection.
...
The observation that proximal (bottom) layers dominate in hierarchical generative models is empirically well-documented in the VAE literature. \citet{sonderby2016ladder} noted that ``VAEs use the lower layers the most whereas the highest layers are not used''; \citet{zhao2017infovae} proved theoretically that ``the first layer contains all the information it needs to generate the data''; and \citet{havtorn2021hierarchical} explicitly stated that ``lowest latent variables learn generic features'' and ``low-level features dominate.'' Our contribution is to \emph{formalize} this observation as a principle with a specific diagnostic implication: for $L$-layer models, IC analysis of only the proximal layer suffices, reducing diagnostic complexity from $O(L^2)$ to $O(1)$.
...
We quantified this relationship empirically. For bivariate Gaussians with varying correlation $\rho$, IC correlates strongly with the angle between Euclidean and natural gradients ($r = 0.90$, $p < 10^{-7}$). At $\IC \approx 0.10$, mean gradient misalignment is $\approx 15^{\circ}$; at $\IC \approx 0.50$, it exceeds $30^{\circ}$.
...
\noindent The relational invariance property shows that for inference diagnostics, only coupling strength matters---not the absolute scale of the investment. A system with $\rho = 0.8$ causes identical MFVI failure whether $I(Z;X) = 0.5$ nats (modest absolute coupling) or $I(Z;X) = 5$ nats (substantial investment).
...
The precision matrix decomposition $\Lambda = D + R$ and the Computational Synergy Principle 
suggest that IC may have diagnostic value beyond continuous Gaussian systems. In discrete 
Boolean domains, analogous structure exists: functions that are affine over finite fields 
exhibit the same ``pairwise-invisible, jointly-determined'' signature that the two-stage 
protocol detects. This raises the possibility that IC---or its natural generalizations---could 
serve as a predictor of approximation difficulty across inference, learning, and control 
problems, wherever methods rely on local or pairwise structure to approximate globally 
coupled systems. The relational structure $R$ that MFVI discards must, in any physical 
implementation, be constructed and maintained by some substrate; understanding how biological 
and engineered systems instantiate and preserve this structure may illuminate both the 
diagnostic's scope and its limitations.
...
\newblock \texttt{doi:10.1016/S0019-9958(57)90116-X}

\bibitem[Gel'fand and Yaglom(1959)]{gelfand1959calculation}
I.~M. Gel'fand and A.~M. Yaglom.
\newblock Calculation of the amount of information about a random function contained in another such function.
\newblock \emph{American Mathematical Society Translations}, 12(2):199--246, 1959.

\bibitem[Wang and Titterington(2004)]{wang2004convergence}
Bo~Wang and D.~Michael Titterington.
\newblock Convergence and asymptotic normality of variational {Bayesian} approximations for exponential family models with missing values.
\newblock In \emph{Proceedings of UAI}, pages 577--584, 2004.
...
\newblock In \emph{Bayesian Time Series Models}, pages 109--130. Cambridge University Press, 2011.

\bibitem[Havtorn et~al.(2021)]{havtorn2021hierarchical}
Jakob~D. Havtorn, Jes Frellsen, S{\o}ren Hauberg, and Lars Maal{\o}e.
\newblock Hierarchical {VAEs} know what they don't know.
\newblock In \emph{Proceedings of ICML}, pages 4117--4128, 2021.

\bibitem[S{\o}nderby et~al.(2016)]{sonderby2016ladder}
Casper~Kaae S{\o}nderby, Tapani Raiko, Lars Maal{\o}e, S{\o}ren~Kaae S{\o}nderby, and Ole Winther.
\newblock Ladder variational autoencoders.
\newblock In \emph{Proceedings of NeurIPS}, pages 3738--3746, 2016.

\bibitem[Zhao et~al.(2017)]{zhao2017infovae}
Shengjia Zhao, Jiaming Song, and Stefano Ermon.
\newblock Info{VAE}: Information maximizing variational autoencoders.
...
\newblock \texttt{doi:10.1007/s11222-016-9696-4}
...
\newblock \texttt{doi:10.2307/2112482}
...
\newblock \texttt{doi:10.1109/TIT.1984.1056927}
...
\newblock \texttt{doi:10.1080/01621459.2017.1285773}
...
\newblock \texttt{doi:10.1109/TSMC.1987.4309069}
...
\newblock \texttt{doi:10.1007/978-4-431-55978-8}
...
\newblock \texttt{doi:10.18653/v1/K16-1002}
...
\newblock \texttt{doi:10.3389/fnhum.2011.00039}
...
\newblock \texttt{doi:10.1126/science.1205438}
...
\newblock \texttt{doi:10.1214/009053607000000505}
...
\newblock \texttt{doi:10.1214/088342306000000024}
...
\newblock \texttt{doi:10.1198/jcgs.2011.203main}
...
\newblock \texttt{doi:10.1016/j.pbiomolbio.2007.03.005}
...
\]
Note: For Gaussian systems, CF reduces to a function of $\IC = |\rho|$ alone, since the marginal entropies cancel in the mutual information (Proposition~\ref{prop:gaussian-mi}).
\end{proposition}
...
\begin{figure}[!htbp]
\centering
\includegraphics[width=\textwidth]{figA5_psis_comparison.pdf}
\caption{\textbf{IC vs.\ PSIS-$\hat{k}$ comparison.} IC correlates strongly with log-likelihood gap ($r = 0.86$), providing a pre-inference diagnostic for MFVI quality. PSIS-$\hat{k}$ values remain negative (indicating reliable importance sampling) even when MSE degradation is substantial, because both MFVI and oracle posteriors are Gaussian with light-tailed importance weights.}
\label{fig:psis-comparison}
\end{figure}
